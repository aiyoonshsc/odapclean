...
- **학습자**: "폴더나 문제 목록의 순서를 내가 원하는 대로(예: 중요도 순) 정렬하고 싶다."
...
- **순서 변경 (Reordering)**: 폴더, 커리큘럼, 문제 목록에서 **드래그 앤 드롭(Drag and Drop)**을 통해 항목의 정렬 순서를 사용자 임의로 변경 가능 (검색 필터링 중에는 비활성화).
...
- `curriculum_id` (FK)
- `title`
- `problem_image_url` (문제 이미지)
- `answer_image_url` (해설/정답 이미지)
- `sort_order` (Integer, Default 0)
- `created_at`, `updated_at`, `created_by`, `updated_by`
*Note: `content` 필드는 이미지 중심의 오답 노트 특성상 제외됨.*
...
2. **Layout**: **상단 네비게이션 바** (Dashboard, My Sessions, Problem Bank, Folders, Curriculums) - 모바일 최적화.
...
5. **Management Pages (Problems, Folders, Curriculums)**:
   - 검색 바, 리스트(드래그 앤 드롭 정렬 지원), 추가/수정 폼(모달 또는 토글), 삭제 버튼.
   - **MainPage UI 개선**: 모바일 최적화된 리스트 뷰(List View) 및 플로팅 액션 버튼(FAB) 적용.
